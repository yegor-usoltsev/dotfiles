...
A one-off prompt for a subagent or a slash command is not a skill: it is read once, by an agent that cannot ask a follow-up question, and it is never triggered by a description. Write it as a briefing with these parts, in this order:
