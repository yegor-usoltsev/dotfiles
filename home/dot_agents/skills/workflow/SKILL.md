...
Use [paseo-messaging](../paseo-messaging/SKILL.md) for peer coordination by default. Brief peers by message or, when the work needs one, a task contract, and follow its role selection and workspace rules. Use [zmx-messaging](../zmx-messaging/SKILL.md) when the user names a live zmx session or requests that transport. If the required transport or review family is unavailable, report the limitation instead of silently omitting review or launching through an improvised path. When the human explicitly rules out the other family but authorizes a same-family peer, use Paseo's reviewer family override in the implementation workspace.
...
Assign review and correction to the other model family by default: Claude reviews and fixes Codex implementation, and Codex reviews and fixes Claude implementation. If the human explicitly requires a same-family reviewer, retain the reviewer role and reuse the implementation workspace; do not launch a builder or create a review worktree. This is an implementation assignment under clean-code; use read-only review only when the user requests it. Carry acceptance criteria and recorded checks into the handoff, including browser scenarios and the code state they verified. Identify the implementing agent by its available return address; a separately launched builder is not required.
